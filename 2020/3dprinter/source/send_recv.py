    ...
    if get_json_data["type"] =="addQueue":       # ok
        ...
        for i in get_json_data["factory"]:       #判断添加任务命令是否是当前工厂
            if int(mach_data["factoryId"]) == i :
                for j in get_json_data["machineCodes"]:       #判断添加任务命令是否是当前工厂
                    if mach_data["machineCode"] == j :
                        #if mach_data["machineCode"] == get_json_data["machineCodes"] :
...
def print_process():
    ...
    while True:
        ...
        #flag表示是否满足空闲状态
        if len(q)>0 and flag == 2 and fetch_product == 0:      #打印队列中至少有一个并且打印机是空闲状态时，发送一个打印请求。否则丢弃这个信号量
    ...
    while True:
        ...
        if ((getinfo.get_isrunning(mach_data["mach_ip"],mach_data["api"]) == 1) or (getinfo.get_isrunning(mach_data["mach_ip"],mach_data["api"]) == 3)) :
            ...
            if fetch_product == 0:
                ...
                mach_info_json["createTime"] = long(time.time())
            else :
                mach_info_json["status"] = 1 
                mach_info_json["fileName"] == ""
                mach_info_json["process"] = 100   
                mach_info_json["printTime"] = 0
                mach_info_json["printTimeLeft"] = 0
                mach_info_json["createTime"] = long(time.time())
        ...
        # mach_info_json["factoryId"] = long(mach_data["factoryId"])
        # mach_info_json["machineCode"] = mach_data["machineCode"]   
        ...
        alarm_info["createTime"] = long(time.time())
        #######################
    ...
    while True:
        ...
        while True:
            ...
            if flag == 2 :  #2代表绝对空闲
                ...
                # overjob["createTime"] = long(time.time())
